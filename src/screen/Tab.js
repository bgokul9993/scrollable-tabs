...
//IMPORTING ACTIONS
import { updateTabs, switchTabs, reorderTabs } from 'redux/actions/app';
...
const mapDispatchToProps = dispatch => ({
    updateTabs: (tabs) => dispatch(updateTabs(tabs)),
    switchTabs: (idx) => dispatch(switchTabs(idx)),
    reorderTabs: (tabs, idx) => dispatch(reorderTabs(tabs, idx))
});
...
class TabListContainer extends Component {
    ...
    componentWillReceiveProps(nextProps) {
        if (!isEqual(nextProps.tabs, this.props.tabs)) {
            this.setState({
                tabTitles: nextProps.tabs.map(item => {
                    return item.title
                }),
                tabs: nextProps.tabs,
            })
    ...
    updateTabs = (updatedTabs, switchTab) => {
        if (switchTab) {
            this.props.switchTabs(updatedTabs.length - 1);
            this.moveLeft();
        }
    ...
    deleteTab = (index) => {
        ...
        index === 0 ? this.props.switchTabs(index) : this.props.switchTabs(index - 1);
    ...
    getTabList = () => {
        const { currentTab } = this.props;
        const { tabTitles, tabs, editIndex } = this.state;
...
export default connect(mapStateToProps, mapDispatchToProps)(TabListContainer);
